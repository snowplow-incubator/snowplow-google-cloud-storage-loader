...
class CloudStorageLoaderSpec extends PipelineSpec {
  ...
  "CloudStorageLoader" should "output a file" in {
    ...
    JobTest[CloudStorageLoader.type]
      .args("--project=test", s"--inputSubscription=${sub}", "--outputDirectory=gs://out-dir/", "--partitionedOutputDirectory=gs://out-dir/partitioned", """--labels={"env": "dev"}""")
  ...
  "getRowType" should "return parsing error when given json is not valid" in {
    val invalidJson =
      """
        ...
        |}
      """.stripMargin
    CloudStorageLoader.getRowType(invalidJson, StaticValueProvider.of(errorDir)) should equal(RowType.Unpartitioned(errorDir))
  }
  ...
  "getRowType" should "return NonSelfDescribing as RowType when given json is not self describing json" in {
    val nonSelfDescribingJson =
      """
        ...
        |}
      """.stripMargin
    CloudStorageLoader.getRowType(nonSelfDescribingJson, StaticValueProvider.of(errorDir)) should equal(RowType.Unpartitioned(errorDir))
  }
  ...
  "getRowType" should "return SelfDescribing as RowType when given json is self describing json" in {
    val json =
      """
        ...
        |}
      """.stripMargin
    CloudStorageLoader.getRowType(json, StaticValueProvider.of(errorDir)).prefix should equal("com.acme2.example1/jsonschema-2")
  }
}

...
class CloudStorageLoaderSpec extends PipelineSpec {
  ...
  "CloudStorageLoader" should "output a file" in {
    ...
    JobTest[CloudStorageLoader.type]
      .args(s"--inputSubscription=${sub}", "--outputDirectory=gs://out-dir/", "--partitionErrorDirectory=gs://out-dir/errors")
  ...
  "getRowType" should "return SelfDescribing as RowType when given json is self describing json" in {
    val json =
      """
        ...
        |}
      """.stripMargin
    CloudStorageLoader.getRowType(json, StaticValueProvider.of(errorDir)).name should equal("com.acme2.example1")
  }
}

...
object CloudStorageLoader {
  ...
  def run(options: Options): Unit = {
    ...
    val partitionedOutputDirectory = options.getPartitionedOutputDirectory()
    ...
    if (partitionedOutputDirectory.isAccessible()) {
      // Partition output according to row type
      val outputDynamic = FileIO.writeDynamic[String, String]()
        .by(getRowType(_, outputDirectory).prefix)
        .via(TextIO.sink())
        .to(partitionedOutputDirectory.get())
  ...
  private[loader] def getRowType(row: String, partitionErrorDir: ValueProvider[String]): RowType =
    parse(row) match {
      case Left(_) => RowType.Unpartitioned(partitionErrorDir.get)
      case Right(json) => SchemaKey.extract(json).fold(
        _ => RowType.Unpartitioned(partitionErrorDir.get),
